...
import { HOME_REPORT } from "../endpoint";
...
const Dashboard = (props: any) => {
  ...
  const fetchData = async (
    ...
    try {
      ...
      setMonth(monthValue);
      setYear(yearValue);
      const { data } = await axios.post(HOME_REPORT, {
        type: value,
        month: monthValue,
        year: yearValue,
      });
      if (data.status === "success") {
        console.log(data.result.graph.label);
        setData(data.result);
      }
    } catch (err) {
      console.log(err);
  ...
  return (
    <Container maxWidth="lg">
      ...
      >
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  ร้าน {props.profile?.prefix?.prefix_name || ""}
                </Typography.Text>
                <Typography.Text
                  style={{ fontSize: "12px", marginLeft: "10px" }}
                >
                  <span style={{ color: "#999999" }}>username : </span>
                  {props.profile?.user_username || ""}
                </Typography.Text>
              </div>
            </Stack>
        ...
        <DashboardCard data={data} type={type} />
      </div>
      <Modal
        open={guide}
